  ...
  const sendClientSMS = async (smsLang: 'FR'|'EN') => {
    ...
    try {
      const response = await fetch('/api/send-sms', {
        ...
        body: JSON.stringify({ to: clientPhone, message, name })
      })
      ...
      {subscription && (
        <div className="dashboard-card2" style={{maxWidth:'1150px', margin:'0 auto 40px auto'}}>
          ...
          <div style={{marginTop:'15px',display:'flex',gap:'15px'}}>
            <button onClick={()=>sendClientSMS('FR')} className="dashboard-button">Envoyer en FR</button>
            <button onClick={()=>sendClientSMS('EN')} className="dashboard-button">Envoyer en EN</button>
          </div>
        </div>
      )}
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
    ...
    {[
      ...
      },
	  {
        ...
        answerFR: "Bonjour {client_name}! Merci pour votre visite chez {business_name}. Votre avis compte énormément pour nous 🙏 Vous pouvez nous laisser un commentaire ici : [LIEN_AVIS - Bonjour {client_name}, suite à votre passage chez {business_name}, nous aimerions connaître votre expérience. Cela ne prend qu’une minute : [LIEN_AVIS - Merci d’avoir choisi {business_name}, {client_name}! Si vous avez apprécié votre visite, un petit avis nous aiderait beaucoup 😊 [LIEN_AVIS] - Salut {client_name}! On espère que vous avez adoré votre expérience chez {business_name} 🤍 Un petit avis ici nous ferait vraiment plaisir : [LIEN_AVIS] - Bonjour {client_name}! Toute l’équipe de {business_name} vous remercie 🙌 Si vous avez 30 secondes, voici le lien pour nous laisser votre avis : [LIEN_AVIS] - Bonjour {client_name}, votre retour nous aide à offrir un service toujours meilleur chez {business_name}. Dites-nous ce que vous en pensez ici : [LIEN_AVIS] - Bonjour {client_name}! Merci d’avoir choisi {business_name}. Cliquez ici pour laisser votre avis : [LIEN_AVIS] ",
        questionEN: "Because we're an extraordinary team, here are some messages already written for you.",
        answerEN: "Hello {client_name}! Thank you for visiting {business_name}. Your opinion means a lot to us 🙏 You can leave us a review here: [REVIEW_LINK] - Hello {client_name}, following your visit to {business_name}, we would like to know about your experience. It only takes a minute: [REVIEW_LINK] - Thank you for choosing {business_name}, {client_name}! If you enjoyed your visit, a short review would help us a lot 😊 [REVIEW_LINK] - Hi {client_name}! We hope you loved your experience at {business_name} 🤍 A short review here would really make us happy: [REVIEW_LINK] - Hello {client_name}! The entire {business_name} team thanks you 🙌 If you have 30 seconds, here's the link to leave us your review: [REVIEW_LINK] - Hello {client_name}, your feedback helps us provide even better service at {business_name}. Tell us what you think here: [REVIEW_LINK] - Hello {client_name}! Thank you for choosing {business_name}. Click here to leave your review: [REVIEW_LINK]"
      }
    ].map((faq, index) => (
      <div key={index} style={{
        background: '#ffffff',
